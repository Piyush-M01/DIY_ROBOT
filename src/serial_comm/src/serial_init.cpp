...
Serial::Serial()
{
  ...
  lt = n.advertise<std_msgs::Float64>("left_ticks", 50);
  rt = n.advertise<std_msgs::Float64>("right_ticks", 50);
  // lt = n.advertise<serial_comm::Ticks>("motor_encoder", 50);
...
void Serial::pub_left_right_ticks()
{
    ...
    else
    {
        ...
        //extracting all the bits and checking if the number of ticks have been received
        //format l+0XXXXXr+0XXXXXX. These values has to be a whole number and positive sign can be replaced by negative sign as sent by the microcontroller
      ...
      //extracts left tick value
      while(l_index<r_index-2)
      {
          left_tick=left_tick+read_buf[l_index];
          l_index++;
      }

      //extracts right tick value
      ...
      std_msgs::Float64 l,r;
      // serial_comm::Ticks enc;
      try
      {
        /* code */

        // checks if the value is positive or negative and publishes to the left_ticks and right_ticks ros topics
        printf("%c %c \n",pol_l,pol_r);

        if(pol_l=='-')
          l.data=-stof(left_tick);
        if(pol_r=='-')
          r.data=-stof(right_tick);
        if(pol_l=='+')
          l.data=stof(left_tick);
        if(pol_r=='+')
          r.data=stof(right_tick);
